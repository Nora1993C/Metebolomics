...
for(k in 1:nrow(Data)){
  Data.Utest[k] <- wilcox.test(Data.Group[[1]][k,],Data.Group[[2]][k,])$p.value
  data.Utest[k] <- wilcox.test(Data.Group[[3]][k,],Data.Group[[2]][k,])$p.value
}
...
write.table(stat,file=paste(outDir,"/",SamID,".U.rev.txt",sep=""),quote = F,sep="\t",row.names = F)
write.table(stat[stat[,8] < 0.05,],file=paste(outDir,"/",SamID,".U.sigrev.txt",sep=""),quote = F,sep="\t",row.names = F)
